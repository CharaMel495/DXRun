...
namespace CaramelEngine
{
    ...
    public:
        Transform(const CVector3& position = {}, const CQuaternion& rotation = CQuaternion::identity(),
            const CVector3& scale = { 1,1,1 }, Transform* parent = nullptr)
            : _position(position), _rotation(rotation), _scale(scale), _parent(parent) 
        { }

        void setPosition(const CVector3& pos) { _position = pos; _dirty = true; }
        void setRotation(const CQuaternion& rot) { _rotation = rot; _dirty = true; }
        void setScale(const CVector3& scl) { _scale = scl; _dirty = true; }
        void setParent(Transform* parent) { _parent = parent; _dirty = true; }
        ...
        {
            using namespace DirectX;
            auto view = XMMatrixLookAtRH(_position.toXMVECTOR(), target.toXMVECTOR(), up.toXMVECTOR());
            _rotation = CQuaternion(XMQuaternionRotationMatrix(XMMatrixTranspose(view)));
            _dirty = true;
        }
        ...
        {
            updateMatrix();
            return _parent ? _localMatrix * _parent->getWorldMatrix() : _localMatrix;
        }
        ...
        std::string toJSON() const 
        {
            ...
            return std::format(
                "{{\"pos\":[{:.3f},{:.3f},{:.3f}],\"rot\":[{:.3f},{:.3f},{:.3f},{:.3f}],\"scale\":[{:.3f},{:.3f},{:.3f}]}}",
